...
public:
    ...
    EventReader(MultiTouchHelper& screen_helper, Gamepad& gamepad) :
        m_Screen(screen_helper), m_Gamepad(gamepad) {}
...
int print_possible_events(int fd, int print_flags)
{
    uint8_t *bits = NULL;
    ssize_t bits_size = 0;
    const char* label;
...
void on_input_event(struct timeval ts, int type, int code, int value)
{
    m_Gamepad.OnEvent(ts, type, code, value);
}

void print_event(int type, int code, int value, int print_flags)
{
...
void print_hid_descriptor(int bus, int vendor, int product)
{
...
int open_device(const char *device, int print_flags)
{
    ...
    struct pollfd *new_ufds = nullptr;
    char **new_device_names = nullptr;
    char name[80];
    char location[80];
    char idstr[80];
    struct input_id id;
        ...
        for(i = 1; i < nfds; i++) {
            if(ufds[i].revents) {
                if(ufds[i].revents & POLLIN) {
                    ...
                    on_input_event(event.time, event.type, event.code, event.value);
                    continue;
...
{
    EventSender screen("/dev/input/event4");
    MultiTouchHelper screen_helper(screen);
    Gamepad gamepad(screen_helper);
    EventReader reader(screen_helper, gamepad);

    char* argv[] {
        _argv[0],
        "/dev/input/event7"
    };
    return reader.getevent_main(2, argv);
}
